...
export async function GET(req: Request, props: { params: Promise<{ id: string }> }) {
    try {
        ...
        const allocations = await pteroClient(`/api/client/servers/${serverId}/network/allocations`, adminClientToken);

        return NextResponse.json({
            allocations: allocations.data.map((a: any) => ({
                id: a.attributes.id,
                ip: a.attributes.ip,
                ip_alias: a.attributes.ip_alias,
        ...
        console.error("Allocations list error:", error.message);
