...
export async function DELETE(req: Request, props: { params: Promise<{ id: string, userId: string }> }) {
    try {
        const { id: serverId, userId } = await props.params;
        ...
        await pteroClient(`/api/client/servers/${serverId}/users/${userId}`, adminClientToken, {
            method: "DELETE"
        });
        ...
        console.error("Subuser delete error:", error.message);
